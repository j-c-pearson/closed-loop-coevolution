"""Make graph showing effect of noise levels on estimation and control performance."""
import os
...
def run_simulation_batch(noise_params=None,
    ...
    noise_params: tuple (float, float) 
        Measurement noise std dev, Process noise std dev for bacteria
        NOTE this only applies to the bacteria process noise; other states have small fixed noise
    """
    figure_number = 'noise_' + label

    SHADING_GRAPHS = False
    POSTER_GRAPHS = False
...
def plot_noise_comparisons(labels,
                           noise_params_list,
                           SHOW_GRAPHS=False,
                           graph_suffix='',
                           graph_labels=None, 
                           ...
                           shading=True):
    """Plot comparison of different noise levels from saved simulation data.

    Args:
        labels: List of labels corresponding to each noise parameter set.
        noise_params_list: List of noise parameter tuples.
    """
    ...
    sns_blue, sns_green, sns_orange, sns_pink, sns_yellow, sns_lightblue = \
        ('#0072B2', '#009E73', '#D55E00', '#CC79A7', '#F0E442', '#56B4E9')
    color_bacteria = sns_blue
    color_inf4 = '#006d2c' # infected
    color_line = '#000000' # line
    color_noise_levels = [sns_blue, sns_green, sns_orange, sns_pink, sns_yellow, sns_lightblue]
    cm = 1/2.54  # centimeters in inches
    
    num_levels = len(noise_params_list)
    fig, axs = plt.subplots(3, 1, figsize=(8.4*cm, 10*cm))
    zorder_list = [2.2, 2.1, 2.0]

    # Process each noise level and compute errors
    for idx, (label, noise_params, zorder) in enumerate(zip(labels, noise_params_list, zorder_list)):
        # Load the results
        data = np.load(f'figures/outputs/figure_noise_{label}.npz')
        t_plot = data['t_plot']
        x_plot = data['x_plot']
        x_hat_plot = data['x_hat_plot']
        ...
        susceptible_true = x_plot_mean[:, 0]
        susceptible_hat = x_hat_plot_mean[:, 0]

        infected_true = (x_plot_mean[:, 1] + x_plot_mean[:, 2] + x_plot_mean[:, 3] +
                        x_plot_mean[:, 4] + x_plot_mean[:, 5] + x_plot_mean[:, 8] +
                        x_plot_mean[:, 9] + x_plot_mean[:, 10] + x_plot_mean[:, 11] +
                        x_plot_mean[:, 12])
        infected_hat = (x_hat_plot_mean[:, 1] + x_hat_plot_mean[:, 2] + x_hat_plot_mean[:, 3] +
                       x_hat_plot_mean[:, 4] + x_hat_plot_mean[:, 5])

        phage_true = x_plot_mean[:, 6] + x_plot_mean[:, 13]
        phage_hat = x_hat_plot_mean[:, 6]

        # # Consider all to get error distributions
        # susceptible_true = x_plot[:, :, 0]
        # susceptible_hat = x_hat_plot[:, :, 0]
        # susceptible_error = np.abs(susceptible_hat - susceptible_true) / (susceptible_true + 1e-6) * 100  # percentage errors
        # susceptible_error_std = np.std(susceptible_error, axis=0)

        # infected_true = (x_plot[:, :, 1] + x_plot[:, :, 2] + x_plot[:, :, 3] +
        #                 x_plot[:, :, 4] + x_plot[:, :, 5] + x_plot[:, :, 8] +
        #                 x_plot[:, :, 9] + x_plot[:, :, 10] + x_plot[:, :, 11] +
        #                 x_plot[:, :, 12])
        # infected_hat = (x_hat_plot[:, :, 1] + x_hat_plot[:, :, 2] + x_hat_plot[:, :, 3] +
        #                x_hat_plot[:, :, 4] + x_hat_plot[:, :, 5])
        # infected_error = np.abs(infected_hat - infected_true) / (infected_true + 1e-6) * 100  # percentage errors
        # infected_error_std = np.std(infected_error, axis=0)

        # phage_true = x_plot[:, :, 6] + x_plot[:, :, 13]
        # phage_hat = x_hat_plot[:, :, 6]
        # phage_error = np.abs(phage_hat - phage_true) / (phage_true + 1e-6) * 100  # percentage errors
        # phage_error_std = np.std(phage_error, axis=0)

        # Compute percentage mean errors (avoid division by zero)
        susceptible_error = 100 * np.abs(susceptible_true - susceptible_hat) / (susceptible_true + 1e-6)
        infected_error = 100 * np.abs(infected_true - infected_hat) / (infected_true + 1e-6)
        phage_error = 100 * np.abs(phage_true - phage_hat) / (phage_true + 1e-6)

        # Subplot 1: Susceptible bacteria error
        axs[0].plot(t_plot, susceptible_error, label=graph_labels[idx],
                    color=color_noise_levels[idx], linewidth=1,
                    zorder=zorder)            

        # Subplot 2: Total infected error
        axs[1].plot(t_plot, infected_error, label=graph_labels[idx],
                    color=color_noise_levels[idx], linewidth=1, 
                    zorder=zorder)

        # Subplot 3: Phage error
        axs[2].plot(t_plot, phage_error, label=graph_labels[idx],
                    color=color_noise_levels[idx], linewidth=1,
                    zorder=zorder)

    # Format subplots
    axs[0].set_ylabel(r'Error in $S_1$ [\%]')
    # axs[0].set_title('Susceptible Bacteria Estimation Error')
    # axs[0].legend(loc='upper right')
    axs[0].set_xticklabels([])
    # axs[0].grid(True, alpha=0.3)

    axs[1].set_ylabel(r'Error in $I$ [\%]')
    # axs[1].set_title('Total Infected Cells Estimation Error')
    # axs[1].legend(loc='upper right')
    axs[1].set_xticklabels([])
    # axs[1].grid(True, alpha=0.3)

    axs[2].set_ylabel(r'Error in $P$ [\%]')
    # axs[2].set_title('Phage Population Estimation Error')
